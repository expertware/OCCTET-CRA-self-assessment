<template>
  <div class="container-fluid">
    ...
    <div class="w-100 d-flex">
      ...
      <div class="container-lg p-0 p-sm-3">
        ...
        <div>
          ...
          <div v-else>
            ...
            <ResultMessageComponent
              ...
              :report="this.report"
              @resultOxySuggestions="SendResultQuestionToOxy"
            ></ResultMessageComponent>
          </div>
        </div>
        ...
        <div
          class="my-2 mt-lg-3 px-3 px-sm-3"
            ...
            <div class="ps-3 border-start">
              <div v-for="(question, indexSectionQuestion) in section.Questions" :key="question.Id">
                <div v-if="question.Type === 'Text'">
                  <Text
                    @AIgenerate="SendQuestionToOxy(question)"
                    :modelValue="question"
                        ...
                        <div class="ps-3 border-start">
                          ...
                          >
                            <div v-if="subQuestion.Type === 'Text'">
                              <Text
                                @AIgenerate="SendQuestionToOxy(subQuestion)"
...
export default {
  ...
  methods: {
    ...
    UpdateSurveyAfterAnswer() {
      ...
      this.$axios
        .get(`/api/Survey/getOrganizationSurvey?${new URLSearchParams(searchParams)}`)
        .then((response) => {
    ...
    async GetSurvey(id) {
      ...
      const api =
        this.$store.state.user.UserName && !this.$store.state.user.UserRoles.includes('Guest')
          ? `/api/Administration/getOrganizationSurvey?organisationSurveyId=${id}`
          : `/api/Survey/getOrganizationSurvey?${new URLSearchParams(searchParams)}`
      await this.$axios
        .get(api)
        .then((response) => {
    ...
    GetSectionsProgress() {
      ...
      const api = !this.$store.state.user.UserRoles.includes('Guest')
        ? `/api/Administration/getSectionProgress?${new URLSearchParams(searchParams)}`
        : `/api/Survey/getSectionProgress?${new URLSearchParams(searchParams)}`
    ...
    async SubmitSurvey() {
      ...
      if (!this.survey.Submited) {
        ...
        if (isFormValid) {
          ...
          this.$axios
            .post(`/api/Survey/submitSurvey?${new URLSearchParams(searchParams)}`)
            .then((response) => {
    ...
    GetReportData(id) {
      ...
      const api =
        this.$store.state.user.UserName && !this.$store.state.user.UserRoles.includes('Guest')
          ...
          : `/api/Survey/getScoreSurveyReports?${new URLSearchParams(searchParams)}`
    ...
    async SendQuestionToOxy(question) {
      ...
      const obj = {
        Prompt:
          `Help the user to answer to this question related to cyber security: "'${question.Title}'"` +
          (question.Answers.length > 0
            ? ` .And i have to choose between: ` +
              question.Answers.map((x) => x.Answer + `${x.Comment ? `- ${x.Comment}` : ''}`).join(
                ', ',
              )
            : '') +
          '. In a nice html and css format only with h4, h5, small, strong, p and ul with li with spaces between sections and not send ```html\n',
      ...
      try {
        const api =
          this.$store.state.user.UserName && !this.$store.state.user.UserRoles.includes('Guest')
            ? `/api/Assistent/sendMessageAdm`
            : `/api/Assistent/sendMessage`
        const response = await this.$axios.post(api, obj, {
    ...
    async SendResultQuestionToOxy(question) {
      this.selectedQuestion = question.Title
        ? question.CategoryOfScore
          ? `Help me understand and improve my score in the category ${question.Title}, where my current score is ${question.Score}.`
          : question.Title
        : `Your score is: ${question.Score}. Here are some recommendations and practical tips to strengthen your security :`
      ...
      const prompt1 = question.Title
          ? `Help the user to understand and help to improve score for category"'${question.Title}'" from ${this.survey.Title} survey` +
            `with score ${question.Score}.` +
            'Give recommendations and best practices tips ' +
            'in a nice html and css format only with h4, h5, small, strong, p and ul with li with spaces between sections and not send ```html\n'
          : `Help the user to understand and help to improve score for ${this.survey.Title} survey with score"` +
            `${question.Score}.` +
            'Give recomadation and best practices tips ' +
            'in a nice html and css format only with h4, h5, small, strong, p and ul with li with spaces between sections and not send ```html\n',
        prompt2 =
          `This is the result for ${this.survey.Title} survey : ${question.Title}. ` +
          'Give a recommendation, explain what it means, and suggest what the user should do next.' +
          'in a nice html and css format only with h4, h5, small, strong, p and ul with li with spaces between sections and not send ```html\n'
      const obj = {
        Prompt: question.Score == 'no-score' ? prompt2 : prompt1,
        ...(this.$route.params.organisationId && {
          OrganisationId: this.$route.params.organisationId,
        }),
      }
      try {
        const api = this.$store.state.user.UserName && !this.$store.state.user.UserRoles.includes('Guest')
          ? `/api/Assistent/sendMessageAdm`
          : `/api/Assistent/sendMessage`
        const response = await this.$axios.post(api, obj, {
          signal: this.abortController.signal,
...
.white-box.active {
  box-shadow: 0px 12px 64px 0px #22a7ff29;
}
.title {
  font-size: 18px;
  font-weight: 600;
